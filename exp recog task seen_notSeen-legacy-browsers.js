...
async function experimentInit() {
  ...
  text_3 = new visual.TextStim({
    ...
    languageStyle: 'LTR',
    color: new util.Color((0.2549, -0.3569, -0.6471)),  opacity: undefined,
    depth: 0.0 
  });
  ...
  seen = new visual.ButtonStim({
    ...
    fillColor: (0.7412, 0.4431, 0.0588),
...
function overall_instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    overall_instructionsComponents.forEach( function(thisComponent) {
    ...
    if (mouse.status === PsychoJS.Status.STARTED) {
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          ...
          gotValidClick = false;
          mouse.clickableObjects = eval([seen, notSeen])
...
function main_task_instructionsRoutineBegin(snapshot) {
  return async function () {
    ...
    main_task_instructionsComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function best_of_luckRoutineBegin(snapshot) {
  return async function () {
    ...
    best_of_luckComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function task_completeRoutineBegin(snapshot) {
  return async function () {
    ...
    task_completeComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
